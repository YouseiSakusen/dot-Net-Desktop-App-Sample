...
public partial class MainWindow : Window
{
	...
	private async void btnMultiInsert_Click(object sender, RoutedEventArgs e)
	{
		var affiliation = new Affiliation() { Id = 19, Name = "見えざる帝国" };

		var characters = new List<Character_Normal>()
		{
			new Character_Normal()
			{
				Name = "ユーハバッハ", Kana = "ユーハバッハ", Sex = Sex.男, Affiliation = affiliation
			},
			new Character_Normal()
			{
				Name = "ユーグラム・ハッシュヴァルト", Kana = "ユーグラムハッシュヴァルト", Sex = Sex.男, Affiliation= affiliation
			}
		};

		var insertCharacters = new List<dynamic>();
		characters.ForEach(c => insertCharacters.Add(new { Name = c.Name, Kana = c.Kana, Sex = c.Sex, AffiliationId = c.Affiliation.Id}));
		...
		await using (var con = new SQLiteConnection(this.connectionStringBuilder.ConnectionString))
		{
			...
			await using (var tran = con.BeginTransaction())
			{
				try
				{
					var count = await con.ExecuteAsync(sql.ToString(), insertCharacters, tran);
					await tran.CommitAsync();
				}
				catch (Exception)
				{
